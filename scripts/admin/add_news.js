$('#company_id').select2({
    placeholder: "Chọn nhà tuyển dụng đăng tin"
});
$('#province').select2({
...
function validate_company() {
    ...
    if ($('#company_id').val().trim() == '') {
        document.getElementById('error_company_id').innerHTML = "Bạn chưa chọn công ty!";
        flag = 0;
    }
}
$('#company_id').on("select2:close", function() {
    validate_company();
})
...
function submit_post() {
    if (form_post_validate()) {
        ...
        $.ajax({
            url: '/Handles/Admin/NewsController/add',
            cache: false,
            contentType: false,
            processData: false,
            type: 'POST',
