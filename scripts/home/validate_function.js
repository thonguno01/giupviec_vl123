...
function validate_phone() {
    ...
    var phoneformat = /((09|03|07|08|05)+([0-9]{8,13})\b)/g;
    if ($('#phone').val() == '') {
...
function validate_password() {
    ...
    if ($('#password').val() == '') {
        ...
        if ($('#password').val().trim().length < 6) {
            document.getElementById('error_password').innerHTML = "Mật khẩu không được ít hơn 6 ký tự!";
            flag = 0;
        } else {
...
function validate_salary() {
    ...
    if ($('#static_sal_btn').is(':checked')) {
        ...
        if ($('#sal_from').val().trim() == '' || $('#sal_from').val().trim() == '0' || $('#sal_to').val().trim() == '' || $('#sal_to').val().trim() == '0') {
            flag = 0;
            document.getElementById('error_sal').innerHTML = "Mức lương không được để trống!"
        }
