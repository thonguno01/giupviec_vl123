...
<div class="main_content">
	...
	<div class="box_left">
		...
		<div class="menu_manager">
			...
			<button id="list_accept" class="active">Người giúp việc đã nhận việc</button>
	...
	<div class="box_dynamic">
		...
		<div class="list_employee_accept">
				...
				<div class="data_table">
					...
					<? foreach ($list_accept as $row) { ?>
						<div class="row">
							...
							<p class="job_location"><?= $row['full_address'] ?></p>
							<p class="job_salary">
							...
							<div class="job_status">
								<button class="change_job_status">
									...
									switch ($row['work_status']) {
										case '1':
											echo '<span class="text working">Đang làm</span>';
											break;
										case '2':
											echo '<span class="text done">Kết thúc</span>';
											break;
										default:
											echo '<span class="text not_start">Chưa bắt đầu</span>';
											break; 
									}
									?>
									<span class="img"><img src="/images/arrow_down.svg" alt="mui ten"></span>
								</button>
								<div class="menu_status" style="display: none;" id="<?=$row['apply_id']?>">
									<button class="not_start <? if ($row['work_status'] == 0) echo 'selected' ?>" value="0">Chưa bắt đầu</button>
									<button class="working <? if ($row['work_status'] == 1) echo 'selected' ?>" value="1">Đang làm</button>
									<button class="done <? if ($row['work_status'] == 2) echo 'selected' ?>" value="2">Kết thúc</button>
								</div>
							</div>
							<div class="job_rate" id="job_rate_<?=$row['apply_id']?>">
								<?
								if($row['work_status']==2){
									if($row['star_rating']!=0){
										echo '<span class="score">
												<div class="score-wrap">
													<span class="stars-active"
														style="width:'.round($row['star_rating']/5*100).'%">
														<i class="fa fa-star"
															aria-hidden="true"></i>
														<i class="fa fa-star"
															aria-hidden="true"></i>
										...
										echo '<button class="open_rate_btn" onclick="rate_action('.$row['apply_id'].')"><img src="/images/employee_rate.svg" alt="danh gia"></button>';
									}
								}
								else{
									echo '<button class="open_rate_btn" disabled><img src="/images/employee_rate_disable.svg" alt="danh gia"></button>';
								}
								?>
							</div>
						</div>
	...
	<div class="popup_bg">
		<div class="popup_content">
			...
			<div class="content">
				...
				<textarea placeholder="Để lại lời nhận xét giúp ứng viên nâng cao thêm chất lượng công việc bạn nha..." id="rate_content"></textarea>
			</div>
			<div class="confirm_button">
